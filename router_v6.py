...
"""
    ...
    with st.spinner("Synthesizing final answer from web + textbooks..."):
        final_answer = call_qwen_stateless(system_prompt, user_prompt) #put medgemma here
...
def create_quiz_pdf(_quiz_run_id: int) -> io.BytesIO:
    ...
    try:
        ...
        for i in sorted(st.session_state.quiz_cache.keys()):
            ...
            # --- NEW CODE (Fixed) ---
            import re
            question_full_text = quiz_data.get("question", "N/A")
            # This isolates the question by splitting the text at the first option (e.g., "A.")
            question_only = re.split(r'\n\s*A\.', question_full_text, 1)[0].strip()
            story.append(Paragraph(question_only.replace("\n", "<br/>"), styles["Normal"]))
